...
new class extends Component {
    ...
    {
        $query = Event::with(['title', 'venue', 'categories', 'user', 'serials'])
            ->active();

        // Apply search filter
        if ($this->search) {
            $query->whereHas('title', function ($q) {
        ...
        if ($this->sortBy === 'title') {
            $query->join('titles', 'events.title_id', '=', 'titles.id')
    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <flux:card class="md:flex md:items-center md:justify-between mb-8">
            ...
            <div class="mt-4 flex md:mt-0 md:ml-4">
                <flux:button variant="primary" href="{{ route('events.create') }}">Create Event</flux:button>
            </div>
        </flux:card>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <div>
                                ...
                                <flux:heading size="xl">{{ Event::active()->where('start_date', '>=', now())->count() }}</flux:heading>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <flux:card>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <!-- Search -->
                    <div class="space-y-3">
                        <flux:input type="text" label="Search" icon="magnifying-glass" placeholder="Search events, venues..." wire:model.debounce.300ms="search" />
                    </div>

                    <!-- Categories -->
                    <div>
                        <flux:select label="Categories" placeholder="Select categories..." wire:model.lazy="selectedCategories">
                ...
                <div class="mt-4 flex flex-wrap items-center gap-4">
                    ...
                    <flux:button variant="primary" size="sm" wire:click="clearFilters">Clear All Filters</flux:button>
                </div>
            </div>
        </flux:card>
            ...
            @forelse($events as $event)
                <flux:card>
                    ...
                    <div class="h-48 bg-gradient-to-r from-teal-500 to-teal-800 relative">
                        @if($event->getMedia('event_files')->where('mime_type', 'like', 'image/%')->first())
                            <img
                                src="{{ $event->getMedia('event_files')->where('mime_type', 'like', 'image/%')->first()->getUrl('thumb') }}"
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center">
                                ...
                                {{ $event->serials->count() }} session{{ $event->serials->count() !== 1 ? 's' : '' }}
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between my-3">
                            <div class="flex space-x-2">
                                <flux:button variant="primary" size="sm" href="{{ route('events.show', $event) }}">Show</flux:button>

                                @can('event-edit')
                                    <flux:button variant="primary" color="amber" size="sm" href="{{ route('events.edit', $event) }}">Edit</flux:button>
                                @endcan
                            </div>

                            @if($event->close_rsvp_date->isFuture())
                                <flux:badge size="sm" color="green" variant="solid">RSVP Open</flux:badge>
                            @else
                ...
                <div class="col-span-full">
                    <div class="text-center py-12">
                        ...
                        <div class="mt-6">
                            ...
                            @else
                                <flux:button href="{{ route('events.create') }}" variant="primary" wire:click="createEvent">Create Event</flux:button>
                            @endif
                        </div>
                    </div>
                </div>
